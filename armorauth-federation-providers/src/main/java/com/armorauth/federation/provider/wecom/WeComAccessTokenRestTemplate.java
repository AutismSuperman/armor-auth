...
public class WeComAccessTokenRestTemplate implements OAuth2AccessTokenRestTemplate {
    ...
    public RestTemplate getRestTemplate(OAuth2AuthorizationCodeGrantRequest authorizationGrantRequest) {
        ...
        tokenResponseHttpMessageConverter.setAccessTokenResponseConverter(responseParameters -> {
            ...
            if (responseParameters.containsKey("expires_in")) {
                responseParameters.put(OAuth2ParameterNames.EXPIRES_IN, responseParameters.get("expires_in"));
            }
            responseParameters.put(OAuth2ParameterNames.CODE, code);
            return new DefaultMapOAuth2AccessTokenResponseConverter().convert(responseParameters);
        });
